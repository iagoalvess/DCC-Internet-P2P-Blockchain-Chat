...
async def receive_archive_response(p2PNode, reader: asyncio.StreamReader):
    ...
    for idx, chat in enumerate(chats.values()):
        ...
        if not verification_check(chat, chats, range(index_before, idx)):
            return
    print_chats(chats, "")
    p2PNode.chats = chats
...
async def put_chat_in_queue(chats, text):
    ...
    for chat in recent_chats:
        bytes_s += (
            chat["length"] + chat["text"] + chat["verification_code"] + chat["md5"]
        )
...
def print_chats(chats: dict, st):
    ...
    for idx, chat in enumerate(chats.values(), start=1):
        ...
        print(
            f"Chat #{idx} -- Texto ({int.from_bytes(chat['length'], byteorder='big')} chars): {text}"
        )
